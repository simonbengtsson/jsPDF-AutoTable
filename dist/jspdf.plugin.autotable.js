...
var jsPDF = __webpack_require__(0);
/**
...
function polyfills_assign(target) {
    'use strict';
    var varArgs = [];
...
function getStringWidth(text, styles) {
    var fontSize = styles.fontSize / state().scaleFactor();
    applyStyles(styles);
    text = Array.isArray(text) ? text : [text];
    var maxWidth = 0;
    text.forEach(function (line) {
        var width = state().doc.getStringUnitWidth(line);
        if (width > maxWidth) {
            maxWidth = width;
        }
    });
    var precision = 10000 * state().scaleFactor();
...
function common_styles(styles) {
    styles = Array.isArray(styles) ? styles : [styles];
    return polyfills_assign.apply(void 0, [defaultStyles()].concat(styles));
}

// CONCATENATED MODULE: ./src/tableDrawer.ts
    ...
    if (remainingPageSpace < row.maxCellHeight) {
        if (remainingPageSpace < getOneRowHeight(row) || (table.settings.rowPageBreak === 'avoid' && !rowHeightGreaterThanMaxTableHeight(row))) {
            ...
            for (var j = 0; j < table.columns.length; j++) {
                ...
                var fontHeight = cell.styles.fontSize / state().scaleFactor() * FONT_ROW_RATIO;
...
function getOneRowHeight(row) {
    return state().table.columns.reduce(function (acc, column) {
        var cell = row.cells[column.dataKey];
        if (!cell)
            return 0;
        var fontHeight = cell.styles.fontSize / state().scaleFactor() * FONT_ROW_RATIO;
...
function nextPage(doc) {
    var current = state().pageNumber();
    doc.setPage(current + 1);
    var newCurrent = state().pageNumber();
    if (newCurrent === current) {
        doc.addPage();
    }
}

// CONCATENATED MODULE: ./src/widthCalculator.ts
...
function calculateWidths(table) {
    ...
    var columnMinWidth = 10 / state().scaleFactor();
...
// CONCATENATED MODULE: ./src/HookData.ts
...
var HookData_HookData = /** @class */ (function () {
    function HookData() {
        var table = state().table;
        this.table = table;
...
var models_assign = __webpack_require__(1);
...
var models_Table = /** @class */ (function () {
    function Table() {
        this.columns = [];
        this.head = [];
        this.body = [];
...
var models_Cell = /** @class */ (function () {
    function Cell(raw, themeStyles, section) {
        ...
        this.styles = models_assign(themeStyles, raw && raw.styles || {});
        this.section = section;
        var text = '';
        ...
        if (typeof this.styles.cellWidth === 'number') {
            ...
            var defaultMinWidth = 10 / state().scaleFactor();
    ...
    Cell.prototype.padding = function (name) {
        var padding = marginOrPadding(this.styles.cellPadding, common_styles([]).cellPadding);
        if (name === 'vertical') {
...
// CONCATENATED MODULE: ./src/cssParser.ts
// Limitations
...
// CONCATENATED MODULE: ./src/htmlParser.ts
...
function parseTableSection(window, sectionElement, includeHidden, useCss) {
    ...
    for (var i = 0; i < sectionElement.rows.length; i++) {
        ...
        var resultRow = [];
        var rowStyles = useCss ? parseCss(row, state().scaleFactor(), ['cellPadding', 'lineWidth', 'lineColor']) : {};
        for (var i_1 = 0; i_1 < row.cells.length; i_1++) {
            var cell = row.cells[i_1];
            var style = window.getComputedStyle(cell);
            if (includeHidden || style.display !== 'none') {
                var cellStyles = useCss ? parseCss(cell, state().scaleFactor()) : {};
                resultRow.push({
...
// CONCATENATED MODULE: ./src/inputValidator.ts

/* harmony default export */ var inputValidator = (function (allOptions) {
...
function parseInput(args) {
    ...
    var documentOptions = getDocumentOptions();
    var allOptions = [globalOptions, documentOptions, tableOptions];
    inputValidator(allOptions);
    var table = new models_Table();
    state().table = table;
    ...
    var _loop_1 = function (styleProp) {
        ...
        table.styles[styleProp] = polyfills_assign.apply(void 0, [{}].concat(styles));
    };
    ...
    table.settings = polyfills_assign.apply(void 0, [{}, defaultConfig()].concat(allOptions));
    ...
    var previous = state().doc.previousAutoTable;
    var isSamePageAsPrevious = previous && previous.startPageNumber + previous.pageNumber - 1 === state().pageNumber();
...
function parseContent(table) {
    ...
    var _loop_2 = function (sectionName) {
        ...
        sectionRows.forEach(function (rawRow, rowIndex) {
            ...
            var row = new Row(rawRow, rowIndex, sectionName);
            ...
            for (var _i = 0, _a = table.columns; _i < _a.length; _i++) {
                ...
                if (rowSpansLeftForColumn[column.dataKey] == null || rowSpansLeftForColumn[column.dataKey].left === 0) {
                    if (columnSpansLeft === 0) {
                        ...
                        var styles = inputParser_cellStyles(sectionName, column.dataKey, rowIndex);
                        var cell = new models_Cell(rawCell, styles, sectionName);
...
function getTableColumns(settings) {
    if (settings.columns) {
        ...
        return dataKeys.map(function (key) { return new Column(key, key, key); });
    }
}
function inputParser_cellStyles(sectionName, dataKey, rowIndex) {
    var table = state().table;
    var theme = getTheme(table.settings.theme);
    ...
    var rowStyles = sectionName === 'body' && rowIndex % 2 === 0 ? polyfills_assign({}, theme.alternateRow, table.styles.alternateRowStyles) : {};
    return polyfills_assign.apply(void 0, [defaultStyles()].concat(otherStyles.concat([rowStyles, colStyles])));
}

// EXTERNAL MODULE: ./src/autoTableText.ts
var autoTableText = __webpack_require__(2);
...
jsPDF.API.autoTable = function () {
    ...
    var table = parseInput(arguments);
